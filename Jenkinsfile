pipeline{
    ...
    stages{
        ...
        stage("Checkout from SCM"){
            steps {
                git branch: 'main', credentialsId: 'github', url: 'https://github.com/shreyas930/Devops-Mega-project.git'
            }
        ...
        stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://65.2.71.61:8080/job/Gitops-devops-mega-project/buildWithParameters?token=gitops-token'"
                }
            }
